...
int bank_check(md_addr_t addr, struct cache_t *cp)
{
    return 0;
}
...
void share_pattern(int threadid, md_addr_t addr, int cmd)
{
    ...
    if (page_table_state == SHARED_WRITE)
    {
        ...
        case PRIVATE_READ:
        {
            SWP_PR_line_access_num++;
            break;
        }
        case PRIVATE_WRITE:
        {
            SWP_PW_line_access_num++;
            break;
        }
        case SHARED_READ:
        {
            SWP_SR_line_access_num++;
            break;
        }
        case SHARED_WRITE:
        {
            SWP_SW_line_access_num++;
            break;
        }
        }
    }
...
int dir_fifo_enqueue(struct DIRECTORY_EVENT *event, int type)
{
    ...
    if (event->operation == INV_MSG_READ || event->operation == INV_MSG_WRITE || event->operation == ACK_DIR_IL1 || event->operation == ACK_DIR_READ_SHARED || event->operation == ACK_DIR_READ_EXCLUSIVE || event->operation == ACK_DIR_WRITE || event->operation == ACK_DIR_WRITEUPDATE || event->operation == FINAL_INV_ACK || event->operation == NACK || event->operation == FAIL)
    {
        ...
        if (event->operation == ACK_DIR_IL1)
            event->when = sim_cycle + cache_il1_lat;
        else
    ...
    else
    {
        ...
        //      event->no_L2_delay = 0;
        //  }
        if ((event->operation == MTA_RMA_READ) || (event->operation == MTA_RMA_WRITE))
        {
            event->when = sim_cycle;
            dir_fifo[threadid][dir_fifo_tail[threadid]] = event;
            dir_fifo_tail[threadid] = (dir_fifo_tail[threadid] + 1) % DIR_FIFO_SIZE;
...
void free_queue(struct DIRECTORY_EVENT *event, struct QUEUE_EVENT *queue)
{
    ...
    for (i = 0; i < QUEUE_SIZE; i++)
    {
        if (queue->Queue_entry[i].isvalid && (queue->Queue_entry[i].event->addr == event->addr) && (queue->Queue_entry[i].event->started == event->started) && (queue->Queue_entry[i].event->missNo == event->missNo))
        {
...
// check if the miss address is already in the mshr; @ fanglei
{
    ...
    for (i = 0; i < mshr_size; i++)
    {
        if (mshr->mshrEntry[i].isValid)
        {
            ...
            {
                if (match != 0)
                    panic("MSHR: a miss address belongs to more than two MSHR Entries");
                match = (i + 1);
            }
        }
    }
...
int mshr_lookup(struct DIRECTORY_EVENT *event, tick_t startcycle)
{
    ...
    for (i = 0; i < mshr_size; i++)
    {
        if (cache_dl1[threadid]->mshr->mshrEntry[i].isValid && ((cache_dl1[threadid]->mshr->mshrEntry[i].addr >> cache_dl1[threadid]->set_shift) == (event->addr >> cache_dl1[threadid]->set_shift)))
        {
...
{
    int tlb_transfer;
    tlb_transfer = event->operation == SHR_TLB_MISS || event->operation == SHR_TLB_WRITE_BACK || event->operation == SHR_TLB_SR_RECOVERY || event->operation == SHR_TLB_UPDATE_PR_ACK || event->operation == SHR_TLB_UPDATE_PW_ACK || event->operation == SHR_TLB_RECOVERY_ACK || event->operation == DIR_TLB_REFILL || event->operation == DIR_TLB_UPDATE || event->operation == DIR_TLB_RECOVERY || event->operation == DIR_TLB_REFILL_WAIT || event->operation == DIR_TLB_UPDATE_WAIT || event->operation == DIR_TLB_RECOVERY_WAIT;
    return tlb_transfer;
}
...
void update_L2_mshrentry(md_addr_t addr, tick_t ready, tick_t startcycle)
{
    ...
    for (i = 0; i < L2_MSHR_SIZE; i++)
    {
        if (cache_dl2->mshr->mshrEntry[i].isValid && (cache_dl2->mshr->mshrEntry[i].addr == addr))
...
int dir_operation(struct DIRECTORY_EVENT *event, int lock_access)
{
    ...
    if (!(event->operation == ACK_DIR_IL1 || event->operation == ACK_DIR_READ_SHARED || event->operation == ACK_DIR_READ_EXCLUSIVE || event->operation == ACK_DIR_WRITE || event->operation == ACK_DIR_WRITEUPDATE || event->operation == SHR_TLB_WRITE_BACK || event->operation == SHR_TLB_SR_RECOVERY || event->operation == DIR_TLB_REFILL))
    {
            ...
            { /* replace the replacement line in L2 cache */
                ...
                for (i = 0; i < iteration; i++)
                {
                    ...
                    { /* send invalidation messages to L1 cache of sharers*/
                        ...
                        if ((ml2_dir_blk_state == MESI_SHARED) && (Is_Shared(repl->dir_sharer[i]) < 2) || (ml2_dir_blk_state == MESI_MODIFIED) && (Is_Shared(repl->dir_sharer[i]) != 1) || (ml2_dir_blk_state == MESI_EXCLUSIVE) && (Is_Shared(repl->dir_sharer[i]) != 1) || (ml2_dir_blk_state == MESI_INVALID))
                        ...
                        if (ml2_hit)
                            if (ml2_ed)
                                if (repl->qblk_set_shift)
                                {
                                    ...
                                    write_S_init_ed_SR_num++;
                            else if (repl->qblk_set_shift)
                            {
        ...
        for (dl1_blk = cp1->sets[set].way_head; dl1_blk; dl1_blk = dl1_blk->way_next)
        {
            if ((dl1_blk->tagid.tag == tag) && (dl1_blk->status & CACHE_BLK_VALID))
            {
                ...
                if (event->parent->sharer_num)
                {
                    ...
                    else if (dl1_blk->state == MESI_MODIFIED)
                        recall_inv_d_S_num++;
                    else if (dl1_blk->state == MESI_EXCLUSIVE)
                        recall_inv_e_S_num++;
                    else
    ...
    case ACK_DIR_READ_SHARED:
        ...
        if ((inform_back_en || (repl->status & CACHE_BLK_DIRTY)) && (repl->status & CACHE_BLK_VALID) && (actualProcess == 1) && !md_text_addr(repl->addr, tempID))
        {
    ...
    case MTA_RMA_WRITE:
    {
        ...
        md_addr_t dstaddr = event->addr;

        int data_int, readData;
        void *p = event->vp;
        void *p_dst = dstaddr;

        int dst_id = (event->des1) * 8 + event->des2;
        int src_id = (event->src1) * 8 + event->src2;
        int dst_threadid = dst_id - 8;
        int src_threadid = src_id - 8;

        while (nbytes--)
        {
            data_int = MEM_READ(thecontexts[src_threadid]->mem, (md_addr_t)p, int);
            MEM_WRITE(thecontexts[dst_threadid]->mem, (md_addr_t)p_dst, int, data_int);

            p += sizeof(int);
            p_dst += sizeof(int);
        }
        printf("RMA write successful!\n");
        fflush(stdout);
        break;
    }
    case MTA_RMA_READ:
    {
        ...
        // generate a reply package send to network
        new_event = allocate_event(0);

        new_event->src1 = event->des1; // swap the src and dst
        new_event->src2 = event->des2;
        new_event->des1 = event->src1;
        new_event->des2 = event->src2;
        new_event->operation = MTA_RMA_READ_REPLY;
        ...
        int data_packet = new_event->nbytes / flit_size + (new_event->nbytes % flit_size > 0 ? 0 : 1);
    ...
    case MTA_RMA_READ_REPLY:
    {
        ...
        // calculate the src and dst threadid
        int dst_id = (event->src1) * 8 + event->src2;
        int src_id = (event->des1) * 8 + event->des2;
        int dst_threadid = dst_id - 8;
        int src_threadid = src_id - 8;
        ...
        {
            /* code */
            data_int = MEM_READ(thecontexts[src_threadid]->mem, (md_addr_t)p_dst, int);
            MEM_WRITE(thecontexts[dst_threadid]->mem, (md_addr_t)p, int, data_int);

            p_dst += sizeof(int);
            p += sizeof(int);
        }

        // recover the thread and update the quiesceAssrStrut info
        if (quiesceAddrStruct[src_threadid].address == (md_addr_t)p_ptr)
        {
            thecontexts[src_threadid]->running = 1;
            thecontexts[src_threadid]->freeze = 0;
            quiesceAddrStruct[src_threadid].address = 1;
            quiesceAddrStruct[src_threadid].threadid = -1;
        }
        break;
    }

    case MTA_DMA_WRITE: // successful
    {
        int data_init, fd;
        dma_message *p_map;
        ...
        fd = open("/home/xiaozhenli/cmp/benchmarks/matrix/write", O_CREAT | O_RDWR | O_APPEND, 00777);

        lseek(fd, sizeof(dma_message) * 100 - 1, SEEK_SET); // extend the size of file to needed by using lseek.
        write(fd, "", 1);

        p_map = (dma_message *)mmap(NULL, sizeof(dma_message) * 100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        close(fd);

        // write n bytes to write file
        while (nbytes--)
        {
            data_init = MEM_READ(thecontexts[src_threadid]->mem, (md_addr_t)p, int);
            (*(p_map + count)).type = 1;      // type = 1 represent the request type is write
            (*(p_map + count)).initiator = 0; // represent the request initiator is slave core
            (*(p_map + count)).srcaddr = -1;
            (*(p_map + count)).dstaddr = dstaddr;
            (*(p_map + count)).data = data_init;
            (*(p_map + count)).deal = 0;
            count++;
            p += sizeof(int);
            dstaddr += sizeof(int);
        ...
        msync((void *)p_map, current_nbytes * sizeof(dma_message), MS_ASYNC);
    ...
    case MTA_DMA_READ:
    {
        ...
        new_event->des1 = event->src1;
        new_event->des2 = event->src2;
        new_event->operation = MTA_DMA_READ_REPLY;
        new_event->nbytes = event->nbytes;
        new_event->addr = event->addr;
        new_event->vp = event->vp;
        new_event->cmd = Read;
    ...
    case MTA_DMA_READ_REPLY:
    {
        ...
        fd = open("/home/xiaozhenli/cmp/benchmarks/matrix/write", O_CREAT | O_RDWR | O_APPEND, 00777); // open the file
        ...
        /**
         * write read request to the "DMA_read" file;
         * @brief
         * the first byte represent the initiator (0 represent the initiator is slave core and 1 represent the initiator is main core)
         * the second byte represent the source address e.g. the address of initiate read request
         * the third byte represent the destination address
         * the forth byte represent the request was replyed or not.
         */
        int nbytes = event->nbytes;
        int currentBytes = event->nbytes;
        md_addr_t dstaddr = event->addr;
        void *p = event->vp;
        void *p_ptr = p;
        int count = 0;
        ...
        while (currentBytes--)
        {
            (*(p_map + count)).type = 0; // type=0 represent the request type is read
            (*(p_map + count)).srcaddr = p;
            (*(p_map + count)).dstaddr = dstaddr;
            (*(p_map + count)).deal = 0;
            (*(p_map + count)).data = -1;
            (*(p_map + count)).initiator = 0; // initiator=0 represent the request initiator is salve core
            p += sizeof(int);
            dstaddr += sizeof(int);
            // printf("read: count is %d the read srcaddr is %d, dstaddr is %d, deal is %d\n", count, (*(p_map + count)).srcaddr, (*(p_map + count)).dstaddr, (*(p_map + count)).deal);
            count++;
        }
        printf("send the once read request\n");

        /**
         * @brief
         * circular wait until the main core reply success.
         */
        // int fd1;
        // dma_message* p_map1;
        // fd1 = open("/home/xiaozhenli/cmp/benchmarks/matrix/write", O_RDWR, 00777);
        // p_map1 = (dma_message *)mmap(NULL, sizeof(dma_message) * 10, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        // close(fd1);
        ...
        //     {
        //         /* code */
        //         while ((*(p_map + reply_count)).dstaddr != 0 && (*(p_map + reply_count)).type == 2)
        //         {
        //             /* code */
        //             p_dstaddr = (*(p_map + reply_count)).dstaddr;
        //             // printf("read: count is %d the read srcaddr is %d, dstaddr is %d, deal is %d\n", reply_count, (*(p_map + reply_count)).srcaddr, (*(p_map + reply_count)).dstaddr, (*(p_map + reply_count)).deal);
        //             if ((*(p_map + reply_count)).deal == 0 && (md_addr_t)p_ptr <= (*(p_map + reply_count)).dstaddr && (md_addr_t)p_ptr + nbytes * sizeof(int) >= (*(p_map + reply_count)).dstaddr) // replyer=0 represent the reply initiator is main core
        //             {
        //                 printf("acccccccccc\n");
        //                 (*(p_map + reply_count)).deal = 1;
        //                 // (*(p_map + reply_count)).type = 3;
        //                 line++;
        //                 printf("deal one successful and the line is %d\n", line);
        //             }
        //             reply_count++;
        //         }
        //         if (line == nbytes)
        ...
        // check the reply file from main core
        if (quiesceAddrStruct[src_threadid].address == (md_addr_t)p_ptr)
        {
            while (1)
            {
                ...
                {
                    p_dstaddr = (*(p_map + reply_count)).dstaddr;
                    if ((*(p_map + reply_count)).deal == 0) // replyer=0 represent the reply initiator is main core
                    {
                        printf("deal is %d and dstaddr is %d and the data is %d\n", (*(p_map + reply_count)).deal, (*(p_map + reply_count)).dstaddr, (*(p_map + reply_count)).data);
                        MEM_WRITE(thecontexts[src_threadid]->mem, (md_addr_t)p_dstaddr, int, (*(p_map + reply_count)).data);
                        int result = MEM_READ(thecontexts[src_threadid]->mem, (md_addr_t)p_dstaddr, int);
                        // fflush(stdout);
                ...
                if (line == nbytes)
                {
                    // recover the thread and update the quiesceAddStruct info
                    quiesceAddrStruct[src_threadid].address = 1;
                    quiesceAddrStruct[src_threadid].threadid = -1;
                    thecontexts[src_threadid]->running = 1;
                    thecontexts[src_threadid]->freeze = 0;
...
cache_access(struct cache_t *cp,                      /* cache to access */
             ...
             int threadid, m_L1WBufEntry *l1_wb_entry // 8 equals thread id..
)
{
    sprintf(dl1name, "dl1");
    ...
    else // level-1 data cache
    {
        ...
        {
        case Read:
            level1_data_cache_access_read_num++; // record the level-1 data cache read num;
            for (blk = cp->sets[set].way_head; blk; blk = blk->way_next)
            {
...
void cache_print(struct cache_t *cp1, struct cache_t *cp2)
...
int simple_cache_flush(struct cache_t *cp)
{
    int i, numflush;
    struct cache_blk_t *blk;
