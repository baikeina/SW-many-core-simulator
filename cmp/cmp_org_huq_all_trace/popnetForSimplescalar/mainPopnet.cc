...
opt_reg_string(odb,"-router_trace","",&routerFile,/*default*/"",/*print*/TRUE,/*format*/NULL); //TengfeiWang 20140224
//
...
void popnetMessageInsert(long s1, long s2, long d1, long d2, long long int sim_cycle, long size, counter_t msgNo, md_addr_t addr, int operation, int vc)
{
	...
	//modified by swain
    flow<<s1<<"  "<<s2<<"   "<<d1<<"  "<<d2<<"  "<< sim_cycle <<"   "<<size<<endl;
	//flow<<s1<<"  "<<s2<<"   "<<d1<<"  "<<d2<<"  "<<msgNo<<"   "<<size<<endl;
    flow.close();  
