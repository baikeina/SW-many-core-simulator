...
#include "instManip.h"
    ...
    /* hq��my counter for stream prefetch */
    ...
    FILE *line_class;
    FILE *vector_distr_file;
    FILE *vector_inc_distr_file;
...
counter_t Total_MetaPackets_1;
counter_t Total_DataPackets_1;
...
//	/* L1 post proved prefetch enable */
    ...
    //#define CPU_TRACE
#ifdef CPU_TRACE
    opt_reg_string(odb,"-cpu:trace","file for cpu pipeline trace record",&cpu_trace,/*default*/"",/*print*/TRUE,/*format*/NULL);
#endif
#ifdef L2_TRACE
    opt_reg_string(odb,"-L2:trace","file for L2 trace record",&L2_trace,/*default*/"",/*print*/TRUE,/*format*/NULL);
#endif
...
//				} /*i added --hq */
			...
			if(rs->writewait != 1)
			{
				...
				committed++;      //tengfei Wang  20140312//
//#ifdef CPU_TRACE
//                FILE *fCpuTrace;
//                fCpuTrace=fopen(cpu_trace,"at+");
//                fprintf(fCpuTrace,"%d  commit  %lld\n",current->id,sim_cycle);
//                fclose(fCpuTrace);
//#endif
				commit_idle = 0;
				for (i = 0; i < MAX_ODEPS; i++)
...
//        FILE *fCpuTrace;
//        fCpuTrace=fopen(cpu_trace,"at+");
//        fprintf(fCpuTrace,"%d  writeback  %lld\n",current->id,sim_cycle);
//        fclose(fCpuTrace);
//#endif
...
#include "machine.def"
                ...
                FILE *fCpuTrace;
                fCpuTrace=fopen(cpu_trace,"at+");
                fprintf(fCpuTrace,"%d  dispatch  %lld\n",current->id,sim_cycle);
                fclose(fCpuTrace);
#endif
	...
	while (tID < numcontexts)
	{
				...
				&& !done && !current->ruu_fetch_issue_delay && current->running && j < (ruu_decode_width * fetch_speed) && current->active_this_cycle /*&& ((ray_mult * (j + current->fetch_num) + current->RUU_num) < (RUU_size - 1))*/; i++, j++)
		{
			...
			if (current->ld_text_base <= current->fetch_regs_PC && current->fetch_regs_PC < (current->ld_text_base + current->ld_text_size) &&/*hq:I suggest remove "!"*/ !(current->fetch_regs_PC & (sizeof (md_inst_t) - 1)))
			{
				...
				{
					MD_FETCH_INST (inst, current->mem, current->fetch_regs_PC);    //tengfei Wang   20140312//

//#ifdef CPU_TRACE
//                    FILE *fCpuTrace;
//                    fCpuTrace=fopen(cpu_trace,"at+");
//                    fprintf(fCpuTrace,"%d  fetch  %lld\n",current->id,sim_cycle);
//                    fclose(fCpuTrace);
//#endif
...
#define PRINT_STATISTICS 0
		...
		if (unPrintLimit < STATUS_NUM && (pure_num_insn >= (unPrintLimit * (max_insts / STATUS_NUM))))
		{
			...
			fflush (stdout);
            // line class
            line_class = fopen ("line_class.txt", "w");
            vector_distr_file = fopen ("vector_distr_file.txt", "w");
            vector_inc_distr_file = fopen ("vector_inc_distr_file.txt", "w");
            int i,j;
            SWP_total_line = 0;
            SWP_sw_line = 0;
            SWP_sr_line = 0;
            ...
            {
                // only valid SW page is recorded; @ fanglei
                if(!page_table[i]->valid)
                    break;
                if(page_table[i]->state != SHARED_WRITE )
                    continue;

                // the lines in SW page is counted; @ fanglei
                for(j=0;j<(MD_PAGE_SIZE/cache_dl1[0]->bsize);j++)
                {
                    if(page_table[i]->line_head[j].line_state != -1)
                        SWP_total_line++;
                    if(page_table[i]->line_head[j].line_state == SHARED_WRITE)
                        SWP_sw_line++;
                    else if(page_table[i]->line_head[j].line_state == SHARED_READ)
                        SWP_sr_line++;
                    else if(page_table[i]->line_head[j].line_state == PRIVATE_WRITE)
                        SWP_pw_line++;
                    else if(page_table[i]->line_head[j].line_state == PRIVATE_READ)
                        SWP_pr_line++;
                }

                // output to recod file; @ fanglei
                fprintf(line_class,"%d:\t", page_table[i]->state );
                for(j=0;j<(MD_PAGE_SIZE/cache_dl1[0]->bsize);j++)
                    fprintf(line_class,"%d\t", page_table[i]->line_head[j].line_state );
                fprintf(line_class,"\n");
            }

            for(i=0;i<cache_ml2->nsets;i++)
            {
                fprintf(vector_distr_file,"%d\t", cache_ml2->sets[i].PVC_vector_distr_record_num );
                for(j=0;j<=cache_ml2->assoc;j++)
                    fprintf(vector_distr_file,"%d\t", cache_ml2->sets[i].PVC_vector_distr[j] );
                fprintf(vector_distr_file,"\n");

                fprintf(vector_inc_distr_file,"%d\t", cache_ml2->sets[i].PVC_vector_inc_distr_record_num );
                for(j=0;j<=cache_ml2->assoc;j++)
                    fprintf(vector_inc_distr_file,"%d\t", cache_ml2->sets[i].PVC_vector_inc_distr[j] );
                fprintf(vector_inc_distr_file,"\n");
            }
            fclose (line_class);
            fclose (vector_distr_file);
            fclose (vector_inc_distr_file);
