...
#include "machine.def"
  ...
  for (;;)
  {
    ...
    if (sim_cycle % 1000 == 0)
    {
      int write_count = 0; // record the DMA write request count
      int read_count = 0;
      ...
      fd = open("/home/xiaozhenli/cmp/benchmarks/matrix/write", O_CREAT | O_RDWR, 00777);
      p_map = (dma_message *)mmap(NULL, sizeof(dma_message) * 100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
      close(fd);

      while ((*(p_map + count)).dstaddr != 0)
      {
        /* code */
        if ((*(p_map + count)).deal == 0 && (*(p_map + count)).type == 1)
        {
          MEM_WRITE(mem, (unsigned long long)(*(p_map + count)).dstaddr, int, (*(p_map + count)).data); // write data to the dstaddress.
          (*(p_map + count)).deal = 1;
        }
        else if ((*(p_map + count)).deal == 0 && (*(p_map + count)).type == 0)
        {
          int request_data = MEM_READ(mem, (unsigned long long)(*(p_map + count)).dstaddr, int);
          (*(p_map + count)).initiator = 1;
          (*(p_map + count)).data = request_data;
          temp = (*(p_map + count)).srcaddr;
          (*(p_map + count)).srcaddr = (*(p_map + count)).dstaddr;
          (*(p_map + count)).dstaddr = temp;
          (*(p_map + count)).type = 2;          
        }
        count++;
      }

      // while ((*(p_map + write_count)).dstaddr != 0 || (*(p_map + read_count)).dstaddr != 0)
      // {
      //   /* code */
      //   /*deal the dma write request*/
      //   if ((*(p_map + write_count)).deal == 0 && (*(p_map + write_count)).type == 1)
      //   {
      //     MEM_WRITE(mem, (unsigned long long)(*(p_map + write_count)).dstaddr, int, (*(p_map + write_count)).data); // write data to the dstaddress.
      //     int requestData = MEM_READ(mem, (unsigned long long)(*(p_map + write_count)).dstaddr, int);
      //     // (*(p_map + write_count)).deal = 1;
      //     // printf("dma write addr is %d and dma write data is %d\n", (*(p_map + write_count)).dstaddr, requestData);
      //     write_count++;
      //   }
      // /*deal the dma read request*/
      // else if ((*(p_map + read_count)).deal == 0 && (*(p_map + read_count)).type == 0)
      // {
      //   // printf("DMA read request in\n");
      //   int request_data = MEM_READ(mem, (unsigned long long)(*(p_map + read_count)).dstaddr, int);

      //   // printf("the request_addr is %d and the data is %d\n", (*(p_map + read_count)).dstaddr, request_data);
      //   (*(p_map + read_count)).initiator = 1;
      //   (*(p_map + read_count)).data = request_data;
      //   temp = (*(p_map + read_count)).srcaddr;
      //   (*(p_map + read_count)).srcaddr = (*(p_map + read_count)).dstaddr;
      //   (*(p_map + read_count)).dstaddr = temp;
      //   (*(p_map + read_count)).type = 2; // type = 2 represent the request type is reply
      //   (*(p_map + read_count)).deal = 0;

      //   // printf("slave core read request deal success\n");
      //   read_count++;
      // }
      // msync((void *)p_map, (read_count + write_count) * sizeof(dma_message), MS_ASYNC);
      // }

      // while ((*(p_map + write_count)).dstaddr != 0) // every cycle check the shared memory
      // {
      //   // printf("the write_count is %d\n", (*(p_map + write_count)).dstaddr);
      //   /* deal the DMA write request */
      //   if ((*(p_map + write_count)).deal == 0 && (*(p_map + write_count)).type == 1)
      //   {
      //     MEM_WRITE(mem, (unsigned long long)(*(p_map + write_count)).dstaddr, int, (*(p_map + write_count)).data); // write data to the dstaddress.
      ...
      // while ((*(p_map + read_count)).dstaddr != 0)
      // {
      //   /* deal the DMA read request */
      //   if ((*(p_map + read_count)).deal == 0 && (*(p_map + read_count)).type == 0)
      //   {
      //     printf("DMA read request in\n");
      //     int request_data = MEM_READ(mem, (unsigned long long)(*(p_map + read_count)).dstaddr, int);
